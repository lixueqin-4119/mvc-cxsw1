...
$tabBar.on("click", "li", function (e) {
  ...
  var index = $li.index();
  localStorage.setItem(localKey, index); //app2.html随意取 无实际意义//设置app2.html为index
...
var $element = (0, _jquery.default)(html).appendTo((0, _jquery.default)('body > .page'));
var $square = (0, _jquery.default)('#app3 .square'); //三个值 yes no undefined;默认是undefined，如果改了呢就是yes
//有结果yes,无结果undefined no.只要排除yes剩下的就是undefined和no
//有结果yes就是true，无结果undefined和no就是false

var localKey = 'app3.active';
var active = localStorage.getItem(localKey) === 'yes'; //等价于const active=localStorage.getItem(localKey) ==='yes' ? true:false

$square.toggleClass('active', active); //等价于上面的
...
$square.on('click', function () {
  if ($square.hasClass('active')) {
    $square.removeClass('active');
    localStorage.setItem(localKey, 'no'); //setItem只支持字符串string,到时候取出来的值为'false';你这时候给个bool值false它会自动转化为字符串false也就是'false'
